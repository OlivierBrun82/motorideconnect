...
class ResetPasswordController extends AbstractController
{
    ...
    public function request(Request $request, MailerInterface $mailer, TranslatorInterface $translator, RateLimiterFactoryInterface $passwordResetLimiter): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            // Anti mail-bombing : on consomme un jeton par IP, avant tout envoi de mail
            ...
            if (!$limit->isAccepted()) {
                ...
                $this->addFlash('danger', sprintf(
                    'Trop de demandes depuis cette connexion. Reessaie dans %d minute%s.',
                    $waitingMinutes,
                    $waitingMinutes > 1 ? 's' : ''
                ));
    ...
    private function processSendingPasswordResetEmail(string $emailFormData, MailerInterface $mailer, TranslatorInterface $translator): RedirectResponse
    {
        ...
        $email = (new TemplatedEmail())
            ...
            ->subject('Réinitialise ton mot de passe sur MotoRide Connect')
